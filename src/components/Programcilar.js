...
export default function Programcilar() {
  ...
  const [programcilarListesi, setProgramcilarListesi] = useState(null);
  const [programciId, setProgramciId] = useState(0);
  ...
  const oneCikaninIsmi = () => {
    ...
    let sonuc = " ";
    enIyilerListesi.forEach((key) => {
      if (programciId === key.id) {
        sonuc = key.isim;
      }
    });
  ...
  const stil = {
    fontSize: "1.5em",
    marginTop: "0.5em",
    color: programciId > 0 ? "gold" : "royalblue", // 🤔 kutlarken renk gold'a dönecek
  };
